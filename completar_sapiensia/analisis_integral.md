...
•	Multi-Instituto para Profesores: Actualmente, el sistema fue concebido para que cada profesor pertenezca a un único instituto (escuela, academia). Sin embargo, en la realidad un profesor podría enseñar en varios lugares. Se quiere soportar que un mismo usuario profesor tenga múltiples afiliaciones (e.g. la Universidad X y el Instituto Y) y pueda manejar sus cursos por separado en cada una. Implicaciones:
•	Al iniciar sesión, si el profesor tiene varias membresías, debería elegir con cuál instituto operar en ese momento. La interfaz podría mostrar un menú para cambiar de contexto (un dropdown con la lista de institutos)[24].
•	A nivel de backend, actualmente el JWT del usuario probablemente incluye un institute_id. Para multi-instituto, hay dos enfoques:
a.	Cambiar el token según el contexto: que al seleccionar instituto, el sistema emita un nuevo token limitado a ese instituto. Esto requiere un endpoint tipo “switch institute” para regenerar JWT[25].
b.	Omitir institute_id en el token y filtrar por membresías en cada request: más flexible pero requiere refactorizar muchas consultas para tener en cuenta posibles múltiples institutos. Quizá demasiado cambio.
•	Dada la premura, se podría optar por la solución sencilla: forzar al usuario a elegir contexto tras login y mantener ese ID en frontend (o en el token) para filtrar datos. Nota: El soporte multi-instituto puede aplazarse o limitarse inicialmente. Incluso se contempló una solución temporal poco elegante: que si es urgente, un profesor use dos cuentas distintas (una por instituto). Sin embargo, la solución definitiva es soportar múltiples roles.
•	También implica que en la vista general no se mezclen cursos de distintos institutos. La lista de cursos debe agruparse o filtrarse por instituto. Por ejemplo, en el dashboard del profesor primero seleccionar la institución, luego ver los cursos de esa institución, etc.
